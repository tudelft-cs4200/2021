...
Make sure to actually append your AST nodes with their types using `@x.type := T` (in a similar way to assigning the `ref` property). Otherwise the rule will not find any types on your AST node.
