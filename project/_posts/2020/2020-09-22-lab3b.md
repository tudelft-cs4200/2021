...
signature
  ...
  constructors
	...
	// you are free to use your own types in your Statix file
	// as long as you write transformation rules from your types to our types
	// in resolve-type

rules
  get-type: node -> type' // Defines a rule which matches on node and returns type'
  	where 
  ...
  resolve-type: a -> a // This rule matches on a and returns a (trivial rule given as example)
...
class Foo(object):
	pass
[[Foo()]]
]] run get-type on #1 to ClassType("Foo")
```
...
- None type: `NoneType()`
- Class with name Foo: `ClassType("Foo")`
- Object: `Object()`
