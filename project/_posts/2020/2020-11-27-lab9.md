...
See the slides of [Lecture 12](/lectures/2020/11/26/lecture12/) on code generation for an approach to get started.

### Objectives

1. Develop tests for your compiler that explore the edge cases.

2. Define a transformation strategy that translates ChocoPy constant expressions to RISC-V code. I.e. including integer, boolean, and string constants and operators.
...
Follow the slides of [Lecture 12](/lectures/2020/11/26/lecture12/) to set up a basic compiler pipeline for translating statement expressions and print the resulting value.
...
Strings in RISC-V are simply pointers to the actual string data somewhere in memory. Therefore, they do not work the same as integers or booleans, on which we can apply direct RISC-V operations in registers. 

We have provided you with another version of the `program-to-rv32im` rule, which has additional boilerplate which includes strings and implementations of operations on strings. 
	...
	program-to-rv32im:
	    ...
	    with
	      ...
	      ; instrs2 := <concat> [
	      	  <asmConstants(|consts)>,
	      	  [PSData()],
	          <initial-class-to-prototype>,
	          <initial-class-to-dispatchTable>,
	          [PSText(), Mv("s10", "gp"), Addi("s11", "s10", "1024")] // NOTE: CURRENTLY THIS LINE IS DIFFERENT IN THE TEMPLATE. THIS IS THE CORRECT ONE!
	        , instrs1
	        , [Li("a0", "1"), 
	           Ecall(), 
	           Li("a0", "10"), 
	           Ecall()] 
	         , <standard-functions(|consts)> // Include the hash table for strings we find in standard functions
	         , [PSData()]
	         , <constants> consts // Map the entries of the hash table into constants
	        ]
```

Strings discovered in ChocoPy source code are pre-compiled into String object constants, which can be found at the bottom of the RISC-V code. In our template implementation, strings encountered throughout the generation process are stored in a Stratego hash table with their label, and at the very end this hash table is read and the strings are turned into constants with RISC-V labels.

1. Have a look at the `trans/generation/defaults.str`, which should provide you with examples on how to add strings to this hash table and get their labels (which represent their addresses). 
2. Update to `stat-to-instrs` and `exp-to-instrs` to include this hash table as a parameter.
3. Update `exp-to-instrs` to include a rule for strings. Have a look at the online compiler or `trans/generation/defaults.str` to find examples of this. The result of this ChocoPy expression should be that the address of our string is loaded into the register in RISC-V. (When adding string `s` to the hash table, make sure to first strip it of its annotations using `<strip-annos> s`, feel free to create auxiliary rules to simplify this process). 
4. Implement the `==`, `!=` and `+` operators for strings. These three operations will involve calling subroutines which take their parameters from the stack. The first and second parameter can be pushed onto the stack using the following RISC-V instructions for now: `sw r 4(sp)` and `sw r 0(sp)` (where `r` is our register) respectively. The output of these subroutines (a 0 or 1 or address to a string) is stored in register `a0`.

In order to print strings and test your implementation, you can implement a transformation for the `Call(Var("print"), [e])` expression. Here we have to call the built-in print function of ChocoPy. This function takes its parameter from the stack, so we have to write our string's address to the stack in RISC-V. For now, this is equivalent to the RISC-V instruction `sw r 0(sp)`, where `r` is our register. This register should hold the address to our string. 
